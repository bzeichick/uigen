...
// Mock the ToolInvocation component
vi.mock("../ToolInvocation", () => ({
  ToolInvocation: ({ toolInvocation }: { toolInvocation: any }) => (
    <div data-testid="tool-invocation">
      {toolInvocation.args?.command === "create" && toolInvocation.args?.path
        ? `Creating ${toolInvocation.args.path.split('/').pop()}`
        : `${toolInvocation.toolName} tool`}
    </div>
  ),
}));
...
test("MessageList renders messages with parts including tool invocations", () => {
  const messages: Message[] = [
    {
      id: "1",
      role: "assistant",
